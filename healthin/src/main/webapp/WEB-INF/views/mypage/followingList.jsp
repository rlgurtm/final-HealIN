...
var category=null;
	function getFollowingList(page){
		$.ajax({
			type:"get",
			url:"${pageContext.request.contextPath}/mypageBothfollow.do",
			data:"pageNo="+page,
			dataType:"json",
...
<ul class="nav nav-tabs follow">
	<li class="menu active">
		<a href="${pageContext.request.contextPath}/userfollowingList.do">팔로잉</a>
	</li>
...
<table class="table table-bordered follow">
	<tbody id="following">
		<c:if test="${!empty list.LVO }">
			...
			<tr>
				<td><span class="followingidnick">${list.id} / ${list.name}&emsp;&emsp;
				</span></td>
				<td>
				<a href="unfollow.do?trainerId=${list.id}">
	...
	<ul class="pagination">
		<c:set var="pb" value="${list.pb}"></c:set>
			<c:if test="${pb.previousPageGroup}">
				<li class="previous"><a href="${pageContext.request.contextPath}/userfollowingList.do?pageNo=${pb.startPageOfPageGroup-1}"> 
				 previous</a></li>	
			</c:if>
			...
			<c:forEach var="i" begin="${pb.startPageOfPageGroup}" end="${pb.endPageOfPageGroup}">
				<c:choose>
					<c:when test="${pb.nowPage!=i}">
						<li><a href="${pageContext.request.contextPath}/userfollowingList.do?pageNo=${i}">${i}</a></li>
					</c:when>
					<c:otherwise>
			...
			<c:if test="${pb.nextPageGroup}">
				<li class="next"><a href="${pageContext.request.contextPath}/userfollowingList.do?pageNo=${pb.endPageOfPageGroup+1}">
				next</a></li>
			</c:if>
	</ul>
</div>
